...
import { getAllFriends, getUserProfile } from "../db/firebaseFunctions";
...
export default function FriendModal(props) {
  ...
  useEffect(() => {
    const grabFriends = async () => {
      ...
      const friendObjects = await Promise.all(friends);
  ...
  const renderFriends = () => {
    ...
    if (searchValue !== "") {
      tempFriends = tempFriends.filter(
        (friend) =>
          friend.displayName.slice(0, searchValue.length).toLowerCase() ===
    ...
    >
      <View style={styles.centeredView}>
        ...
        <TextInput
          style={[
            global.input,
            { width: 300, marginTop: 20, marginVertical: 20 },
          ]}
